    ...
    return (
      <main className="min-h-screen bg-[linear-gradient(180deg,#f4f8ff_0%,#ffffff_100%)]">
        ...
        <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
          ...
          <Link
            to="/admin/employees"
            className="inline-flex items-center gap-2 text-sm font-semibold text-[#1DA1F2] hover:gap-3 transition-all"
          >
      ...
      <section className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <section className="rounded-3xl border border-[#E5E7EB] bg-white p-6 md:p-8 shadow-[0_24px_40px_-34px_rgba(11,61,145,0.6)]">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <div>
              <h1 className="text-3xl font-heading font-bold text-[#0B3D91]">
                {employee.fullName}
              </h1>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => copyUID(employee.uid)}
                className="rounded-lg border border-[#E5E7EB] p-2 text-[#0B3D91] hover:bg-[#1DA1F2]/10 transition"
              >
                <Copy className="h-4 w-4" />
              </button>
              <span
                className={`text-xs font-semibold transition-opacity ${uidCopied ? "opacity-100" : "opacity-0"}`}
              >
                Copied!
              </span>
            </div>
        ...
        <Card title="Responsibilities">
          <p className="text-slate-700 whitespace-pre-wrap">
            {employee.responsibilities || "No responsibilities specified."}
          </p>
        </Card>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="rounded-lg border border-[#E5E7EB] bg-slate-50 p-3">
                  ...
                  <p className="mt-1 text-2xl font-bold text-[#1DA1F2]">
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold border ${styles[status]} border-current border-opacity-30`}
    >
      {status}
    </span>
  );

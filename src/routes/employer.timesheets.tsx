...
function EmployerTimesheetsPage() {
  ...
  function onEmployerReject(record: TimesheetRecord) {
    ...
    const next = timesheets.map((row) =>
      row.id === record.id
        ? rejectTimesheetByRole(row, "employer", "Employer", reason ?? undefined)
        : row,
    );
    commit(next);
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div>
          ...
          <p className="mt-1 text-slate-600">
            View and review timesheets related to assigned resources.
          </p>
        </div>
              ...
              {rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <tr key={row.id} className="border-b border-[#E5E7EB]/70">
                    ...
                    <td className="py-3">
                      {row.weekStartDate} to {row.weekEndDate}
                    </td>
                    ...
                    <td className="py-3">
                      <div className="flex gap-2">
                        <button
                          type="button"
                          className="rounded-lg border border-[#E5E7EB] px-2 py-1 text-xs font-semibold text-[#0B3D91]"
                        >
                          View
                        </button>
                        <button
                          type="button"
                          disabled={
                            row.status !== "Submitted" || row.employerApprovalStatus !== "Pending"
                          }
                          onClick={() => onEmployerApprove(row)}

...
function ClientTimesheetsPage() {
  ...
  function onReject(record: TimesheetRecord) {
    ...
    const next = timesheets.map((row) =>
      row.id === record.id
        ? rejectTimesheetByRole(row, "client", "Client", reason ?? undefined)
        : row,
    );
    commit(next);
      ...
      <section className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div>
          ...
          <p className="mt-1 text-slate-600">
            Review and approve submitted timesheets for assigned resources.
          </p>
        </div>
                    ...
                    <td className="py-3">
                      <div className="flex gap-2">
                        ...
                        <button
                          type="button"
                          disabled={
                            row.status !== "Submitted" || row.clientApprovalStatus !== "Pending"
                          }

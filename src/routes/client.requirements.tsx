...
function ClientRequirementsPage() {
  ...
  const commit = useCallback((next: ClientRequirementRecord[]) => {
  ...
  const loadRequirements = useCallback(async () => {
    try {
      ...
      const result = await listClientRequirementsFn({
        data: {},
        headers: {
      ...
      setMessage(error instanceof Error ? error.message : "Unable to load client requirements.");
      return;
    }
  ...
  function validate(values: ClientRequirementInput): FormErrors {
    ...
    if (!values.requiredStartDate.trim())
      next.requiredStartDate = "Required Start Date is required.";
    return next;
  }
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : "Unable to save client requirement.");
      return;
    }
  }
              ...
              <Field label="Service Domain">
                <select
                  ...
                  onChange={(e) =>
                    setField(
                      "serviceDomain",
                      e.target.value as ClientRequirementInput["serviceDomain"],
                    )
                  }
                >
            ...
            <div className="grid md:grid-cols-3 gap-4">
              ...
              <Field label="Work Mode">
                <select
                  ...
                  value={form.workMode}
                  onChange={(e) =>
                    setField("workMode", e.target.value as ClientRequirementInput["workMode"])
                  }
                >
              ...
              <Field label="Required Start Date" error={errors.requiredStartDate}>
                <input
                  type="date"
                  className={inputClass(errors.requiredStartDate)}
                  value={form.requiredStartDate}
                  onChange={(e) => setField("requiredStartDate", e.target.value)}
                />
              </Field>
            </div>
            <div className="grid md:grid-cols-3 gap-4">
              <Field label="Expected Duration">
                <input
                  ...
                  onChange={(e) => setField("expectedDuration", e.target.value)}
                />
              </Field>
              <Field label="Resources Needed">
                <input
                  ...
                  min={1}
                  className={inputClass()}
                  value={form.numberOfResourcesNeeded}
                  onChange={(e) => setField("numberOfResourcesNeeded", Number(e.target.value) || 1)}
                />
              </Field>
              <Field label="Budget / Rate Range">
                <input
                  className={inputClass()}
                  value={form.budgetRateRange}
                  onChange={(e) => setField("budgetRateRange", e.target.value)}
                />
              </Field>
            </div>
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="Priority">
                <select
                  ...
                  value={form.priority}
                  onChange={(e) =>
                    setField("priority", e.target.value as ClientRequirementInput["priority"])
                  }
                >
              ...
              <Field label="Status">
                <select
                  ...
                  value={form.status}
                  onChange={(e) =>
                    setField("status", e.target.value as ClientRequirementInput["status"])
                  }
                >
                  {statuses.map((status) => (
                ...
                requirements.map((item) => (
                  <div key={item.id} className="rounded-xl border border-[#E5E7EB] p-3">
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      {item.clientName} • {item.serviceDomain}
                    </p>
